...
// Correct answers, concurrent, takes about 300s to compute.
...
func (z *Case) solveSmall() interface{} {
	...
	for i := 0; i < z.N-1; i++ {
		for j := i + 1; j < z.N; j++ {
			minM := width - z.R[i] - z.R[j] + 1
			if minM > z.M {
				continue
			}
			groupOrder := facto[z.N-2]

			holes := z.M - minM
			combiHoles := binom(z.N+1+holes-1, z.N+1-1) // stars and bars
