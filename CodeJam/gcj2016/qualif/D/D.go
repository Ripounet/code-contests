...
func (z *Case) solveSmall() interface{} {
	...
	gap := pow(z.K, z.C-1)
	if z.C == 1 {
		gap = 0 // this is an exception but...
	}
	s := ""
	glue := ""
	for i := 0; i < z.S; i++ {
		s += fmt.Sprintf("%v%d", glue, 1+(i*gap+i))
		glue = " "
	}
	return s
}
...
func (z *Case) solveLarge() interface{} {
	...
	X := (z.K + (z.C - 1)) / z.C
	positions := make([]*big.Int, X)
	tile := 0
	for x := range positions {
		pos := new(big.Int)
		for c := 0; c < z.C; c++ {
			gap := big.NewInt(int64(z.K))           // gap is K
			gap.Exp(gap, big.NewInt(int64(c)), nil) // gap is K^c
			gap.Mul(gap, big.NewInt(int64(tile)))   // gap is (K^c) * tile
			pos.Add(pos, gap)
			tile++
			tile %= z.K // extra evidence discounted
		}
		positions[x] = big.NewInt(1) // tiles are 1-based
		positions[x].Add(positions[x], pos)
	}
	y := fmt.Sprintf("%v", positions)
